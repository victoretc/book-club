...
class Configuration:
    def __init__(self, *, base_url: URL | str = "", disable_log: bool = False, disable_allure_log: bool = False, **kwargs: Any) -> None:
        """
        Parameters
        ----------
            base_url (optional):
...
class AllureLogging:
    def __init__(self, configuration: Any) -> None:
        ...
        self.env = Environment(loader=PackageLoader("app.testing"), autoescape=select_autoescape(), trim_blocks=True, lstrip_blocks=True)
    ...
    def log_to_allure(self, response: httpx.Response) -> None:
        ...
        request_html = self._render_template("http-request.html", {"request": request_data, "curl": curl_command})
        ...
        response_html = self._render_template("http-response.html", {"response": response_data})

        with allure.step(f"{response.request.method} {response.request.url}"):
            allure.attach(body=request_html, name="Request", attachment_type=AttachmentType.HTML, extension=".html")
            allure.attach(body=response_html, name=f"Response {response.status_code}", attachment_type=AttachmentType.HTML, extension=".html")
